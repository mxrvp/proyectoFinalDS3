...
public class PanelEmpleadoPlanilla extends javax.swing.JPanel {
   private PlanillaModel pMod=new PlanillaModel();
    int  statusbtnAddEmpleadoToPlanilla=0;
    ...
    private void initComponents() {
        ...
        btnAddEmpleadoToPlanilla.setEnabled(false);
    ...
    private void btnAddEmpleadoToPlanillaMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnAddEmpleadoToPlanillaMousePressed
        ...
        if(this.statusbtnAddEmpleadoToPlanilla==1){
          ...
          if(errors.length()>0){   
             ...
             Utilitario.erro(msg,null);
              ...
              if(pMod.insertDetalle(pMod.getLastInsert(),txtBuscarCedula.getText(),Double.parseDouble(txtHrsTrabajadas.getText()),Double.parseDouble(txtSalxHora.getText()))){
                  Utilitario.exi("El empleado ha sido agregado a la planilla con éxito!", null);
                   limpiarForm();
              }
 ...
 CRUD_Empleado obj = new CRUD_Empleado();
        ...
        if (obj.BuscarEmpleado()) {
           ...
           Utilitario.erro("No se encontro el empleado", null);
        }
        ...
        String sH=txtSalxHora.getText();
          ...
          String def[]={
           ...
           for(int i=0;i<=ar.length-1;i++){
            ...
            if(!Utilitario.estaVacia(hT)){
                
               if(Utilitario.esNumero(hT)){
                     if(!evalHoras(Double.parseDouble(hT))){
                       err+="Las horas trabajadas no pueden ser menores a 4 ni mayores a 12";
                      err+="\n";
                   }
               }
               else{
                     err+="Las horas trabajadas deben ser numéricas";
                      err+="\n";
               }
            ...
            if(!Utilitario.estaVacia(sH)){    
                  if(Utilitario.esNumero(sH)){
                     if(!evalSalario(Double.parseDouble(sH))){
                       err+="Las horas trabajadas no pueden ser menores a 5 ni mayores a 20";
                      err+="\n";
                   }
                      
               }
                  else{
                     err+="El salario debe ser numérico";
                      err+="\n";
                  }
    ...
    public void limpiarForm(){
      ...
      public void onPn(){
        ...
        public void onbtnAddEmpleadoToPlanilla(){
        btnAddEmpleadoToPlanilla.setEnabled(true);
        this.statusbtnAddEmpleadoToPlanilla=1;
    }
       
        public void offbtnAddEmpleadoToPlanilla(){
               btnAddEmpleadoToPlanilla.setEnabled(false);
        this.statusbtnAddEmpleadoToPlanilla=0;
    }
